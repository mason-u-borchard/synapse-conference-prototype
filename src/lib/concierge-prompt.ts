...
export function assembleConciergePrompt(provider: "anthropic" | "openai" | "none"): string {
  ...
  return [
    ...
    `Fiscal sponsor (placeholder -- committee is finalizing a 501(c)(3) partnership): ${meta.fiscalSponsor.name} (${meta.fiscalSponsor.short})`,
    `Fiscal sponsor blurb: ${meta.fiscalSponsor.blurb}`,
    `When asked about the fiscal sponsor, be explicit that the name shown on the site is a placeholder while the committee finalizes the partnership; do not invent a specific organization.`,
    ``,
