...
function renderConfirmationHtml({ fullName, confirmationId }: { fullName: string; confirmationId: string }): string {
  ...
  <html><body style="font-family: 'Georgia', serif; background: #faf6ef; color: #1e0e22; padding: 40px;">
    <div style="max-width: 540px; margin: 0 auto;">
      ...
      <p style="font-size: 14px; color: #6b5a70; margin-top: 32px;">${meta.dates.display} &middot; ${meta.city} &middot; Fiscally sponsored by ${meta.fiscalSponsor.name}</p>
    </div>
  </body></html>
  `;
}
...
Fiscally sponsored by ${meta.fiscalSponsor.name}`;
}
